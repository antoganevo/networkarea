...
while($data = mysqli_fetch_array($sql))
{
    ...
    $pdf->Ln(5);
    $pdf->Cell(5);
    $pdf->cell(15,9,'Perangkat yang dimaksud dengan rincian modul sebagai berikut :');
    $pdf->Ln(8);
    $pdf->Cell(5);
    $pdf->SetFont('Arial','B',7);
    $pdf->Cell(10,9,'No.',1,0,'C');
    $pdf->Cell(55,9,'Vendor',1,0,'C');
    $pdf->Cell(42,9,'Type Board',1,0,'C');
    $pdf->Cell(47,9,'Serial Number',1,0,'C');
    $sql =  $conn->query("SELECT * FROM modulk WHERE iduniq = '$iduniq'");
    $i = 1;
    while($data = mysqli_fetch_array($sql))
    {
        $pdf->SetFont('Arial','',7);
        ...
        $pdf->Cell(55,9,''.$data['vendor'].'',1,0,'C');
        $pdf->Cell(42,9,''.$data['tboardk'].'',1,0,'C');
        $pdf->Cell(47,9,''.$data['snk'].'',1,0,'C');
    }
}
