...
while($data = mysqli_fetch_array($sql))
{
    ...
    $pdf->Cell(30,9,'Jenis',1,0,'C');
    $pdf->Cell(25,9,'Vendor',1,0,'C');
    $pdf->Cell(20,9,'Bandwidth (G)',1,0,'C');
    $pdf->Cell(25,9,'Lambda (nm)',1,0,'C');
    $pdf->Cell(20,9,'Jarak (Km)',1,0,'C');
    $pdf->Cell(40,9,'Serial Number',1,0,'C');
    $sql =  $conn->query("SELECT * FROM sfpk WHERE iduniq = '$iduniq'");
    $i = 1;
    while($data = mysqli_fetch_array($sql))
    {
        $pdf->SetFont('Arial','',7);
        ...
        $pdf->Cell(30,9,''.$data['jenisk'].'',1,0,'C');
        $pdf->Cell(25,9,''.$data['vendork'].'',1,0,'C');
        $pdf->Cell(20,9,''.$data['bandwidthk'].'',1,0,'C');
        $pdf->Cell(25,9,''.$data['lambdak'].'',1,0,'C');
        $pdf->Cell(20,9,''.$data['jarakk'].'',1,0,'C');
        $pdf->Cell(40,9,''.$data['snk'].'',1,0,'C');
    }
